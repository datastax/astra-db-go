...
func (c *CustomWithPointer) ToAstraRaw(_ targetKind, dst []byte) ([]byte, error) {
	...
	return SerializeInto(m, CollectionTarget, dst)
}

func (c *CustomWithPointer) FromAstraRaw(_ targetKind, val []byte) error {
	var m map[string]any
	if err := Deserialize(val, &m, CollectionTarget); err != nil {
		return err
	}
	if val, ok := m["value"].(string); ok {
		c.Value = val
...
func TestSerdesCustom(t *testing.T) {
	str, err := Serialize(CustomWithValue{Value: "hello"}, CollectionTarget)
	if err != nil {
	...
	src, err := Serialize(&CustomWithValue{Value: "hello"}, CollectionTarget)
	if err != nil {
	...
	str, err = Serialize(&CustomWithPointer{Value: "world"}, CollectionTarget)
	if err != nil {
	...
	if err := Deserialize([]byte(`{"value":"hello"}`), &dstVal, CollectionTarget); err != nil {
	...
	if err := Deserialize([]byte(`{"value":"world"}`), &dstPtr, CollectionTarget); err != nil {
