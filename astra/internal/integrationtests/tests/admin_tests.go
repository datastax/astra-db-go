...
func AdminCreateDropDatabase(e *harness.TestEnv) error {
	...
	// Verify we can get the database info via AstraDatabaseAdmin.
...
func AdminKeyspaceCreateListDrop(e *harness.TestEnv) error {
	...
	// Get an AstraDatabaseAdmin for this database
	dbAdmin := admin.DatabaseAdminFromEndpoint(db.Regions[0].APIEndpoint, nil)
...
func AdminGetDatabaseNotFound(e *harness.TestEnv) error {
	...
	if !errors.Is(err, astra.ErrNotFound) {
