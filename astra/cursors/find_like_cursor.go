...
func (c *findLikeCursorImpl[Raw]) fetchNextPage(ctx context.Context) (bool, error) {
	...
	if err := serdes.Deserialize(b, &resp, nil, c.target, c.fcs.apiOptions().GetDesFlags()); err != nil {
		c.currentPage = nil
		return false, err
	}
	...
	if resp.Status != nil && resp.Status.SortVector != nil {
		c.sortVector = resp.Status.SortVector
	}
